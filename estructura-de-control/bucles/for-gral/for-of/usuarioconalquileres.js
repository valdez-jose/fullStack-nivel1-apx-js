...
for (const usuario of usuarios) {
  ...
  for (const publicacion of usuario.publicaciones) {
    console.log("Revisando la publicación:", publicacion.title);

    if (publicacion.type === "alquiler") {
      idsDeUsuarioConAlquileres[usuario.id] = true;
      console.log(
        `El usuario ${usuario.id} tiene una publicación de alquiler: ${publicacion.title}`
      );
      // Si ya encontramos una publicación de alquiler, no necesitamos seguir buscando en este usuario
      break;
    }
  }
}
